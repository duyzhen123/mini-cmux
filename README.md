...
mini-cmux events [--follow] [--project PROJECT] [--agent AGENT]
...
Pane capture is bounded to 2,000 lines. The watcher scans the most recent 250
lines for markers, so agents should emit a final structured marker and the
watcher should run during long unattended jobs.
